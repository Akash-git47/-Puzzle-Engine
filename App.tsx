...
const App: React.FC = () => {
  ...
  const handleMovePiece = useCallback((pieceId: string, requestedDir?: Direction) => {
    setGameState(prev => {
      // Logic change: We no longer block movement if isSolved is true.
      // This allows the user to "keep on playing" as requested.
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
      ...
      {/* Stats Bar */}
      <div className="w-full max-w-sm flex justify-between items-center mb-8 px-6 py-4 bg-white shadow-xl shadow-slate-200/50 rounded-2xl border border-slate-100">
      ...
      {/* Victory modal block removed to prevent interruptions */}

      {/* Footer Info */}
      <div className="mt-10 text-center opacity-40">
         <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500">Classic Mechanics • Minimalist Design</p>
      </div>
    </div>
  );
};
